...
for r in range(repeats):
    # fit 模型
    lstm_model = fit_lstm(train_scaled, 1, 100, 26)  # 训练数据，batch_size，epoche次数， 神经元个数
    # 预测
...
def lstm_fitness_function(params):
    batch_size = int(params[0])  # 批量大小
    nb_epoch = int(params[1])  # 训练轮数
    neurons = int(params[2])  # 神经元数量
    ...
    # 计算RMSE
    rmse = sqrt(mean_squared_error(raw_values[-12:], predictions))
    print(f"当前超参数组合: batch_size={batch_size}, nb_epoch={nb_epoch}, neurons={neurons}, RMSE={rmse}")
    return rmse  # 返回RMSE，越小越好


# 设置粒子群优化的边界
# 这里的边界是批量大小、训练轮数和神经元数量的范围
lb = [1, 10, 1]  # 批量大小范围[1, 10]，训练轮数范围[10, 100]，神经元范围[1, 100]
ub = [10, 100, 100]  # 批量大小最大值为10，训练轮数最大值为100，神经元最大值为100

# 使用粒子群算法优化LSTM超参数
best_params, best_rmse = pso(lstm_fitness_function, lb, ub, swarmsize=10, maxiter=5)

# 打印最优超参数和对应的RMSE
print("最优的超参数组合: ")
print(f"批量大小: {int(best_params[0])}, 训练轮数: {int(best_params[1])}, 神经元数量: {int(best_params[2])}")
